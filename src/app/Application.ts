...
type GameId = "beard-bank" | "blackjack" | "roulette" | "free-drop" | "neema";
...
export class Application {
    ...
    this.appRoot.innerHTML = `
      <section class="casino-shell">
        ...
        <div class="game-grid">
          ...
          ${this.gameCard("neema", "COMING NEXT", "NEEMA'S SLOTS", "A joyful slot built for Mom", "preview rose")}
        </div>
      </section>`;
  ...
  private openGame(id: GameId): void {
    ...
    this.openPreview("NEEMA'S SLOTS", "This one deserves her personality, favorite themes, sounds, colors, and bonus style. The room is reserved. The generic machine is not.");
  }

  private async openBeardBank(): Promise<void> {
    this.appRoot.innerHTML = "";
    this.pixi = new PixiApplication();
    await Promise.all([this.pixi.init({ resizeTo: window, background: new Color(0x12081f), antialias: true }), Assets.load([cabinetAssetUrl, ...symbolAssetUrls])]);
  ...
  private openPreview(title: string, copy: string): void {
    this.appRoot.innerHTML = `<section class="preview-room"><button class="back" data-back>← LOBBY</button><small>RESERVED ON THE CASINO FLOOR</small><h1>${title}</h1><p>${copy}</p><div class="coming-machine">COMING NEXT</div></section>`;
    this.appRoot.querySelector("[data-back]")?.addEventListener("click", () => this.showLobby());
  }
