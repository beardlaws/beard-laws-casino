...
export class ReelStripView{
 ...
 async mount(host){
  ...
  await Promise.all(ids.map(async id=>{this.textures[id]=await Assets.load(`./assets/${id}.svg?v=3.1.0`);}));
 ...
 clearWinPresentation(){
  for(const reel of this.reels){
   ...
   for(const holder of strip.children){
    holder.alpha=1;
    holder.scale.set(1);
 ...
 showWinningPositions(positions){
  ...
  this.reels.forEach((reel,reelIndex)=>{
   const strip=reel._strip;
   strip.children.forEach((holder,rowIndex)=>{
    const winning=winners.has(`${reelIndex}-${rowIndex}`);
    holder.alpha=winning?1:.35;
    holder.scale.set(winning?1.035:1);
    const visual=holder._reelVisual;
    if(visual){
     visual.outer.tint=winning?0xfff1a0:0x777777;
