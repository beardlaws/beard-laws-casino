...
const SYMBOLS: readonly JamSymbol[] = [
  ...
  { id: "megh", label: "MEGH", art: art("megh"), weight: 7, pay: 30 },
...
export class MeghsCosmicJam {
  ...
  public open(): void {
    this.root.innerHTML = `<main class="megh-room"><button class="back" data-megh-home>← CASINO LOBBY</button>
      ...
      <section class="megh-machine"><div class="laser-grid"></div><div class="megh-top">
  ...
  private render(grid: JamSymbol[][], winners = new Set<string>()): void {
    ...
    host.innerHTML=grid.flatMap((row,y)=>row.map((symbol,x)=>`<div class="jam-symbol s-${symbol.id}${winners.has(`${x}:${y}`)?" winner":""}" style="grid-column:${x+1};grid-row:${y+1}"><span>${symbol.label}</span><img src="${symbol.art}" alt="${symbol.label}" onerror="this.hidden=true;this.parentElement.classList.add('art-failed')"><small>${symbol.label}</small></div>`)).join("");
  }
