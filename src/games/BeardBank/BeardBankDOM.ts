...
export class BeardBankDOM {
  ...
  private async spin(): Promise<boolean> {
    ...
    this.say("VAULT WHEELS IN MOTION");
  ...
  private async vaultHeist(bet: number, coins: number): Promise<void> { window.dispatchEvent(new CustomEvent("casino:sound",{detail:{cue:"vault"}})); this.onActivity({ type: "bonus", game: "beard-bank" }); this.root.querySelector(".bb60-machine")?.classList.add("feature-mode"); this.say("VAULT HEIST • LOCKS ENGAGED"); await wait(1200); const award = Math.round(bet * (3 + coins * 1.5 + casinoRandom() * 8)); await this.countAward(award, "HEIST COMPLETE"); this.root.querySelector(".bb60-machine")?.classList.remove("feature-mode"); }
  private async vernonSpins(bet: number, spins: number): Promise<void> { window.dispatchEvent(new CustomEvent("casino:sound",{detail:{cue:"vernon"}})); this.onActivity({ type: "bonus", game: "beard-bank" }); this.say(`VERNON FREE SPINS • ${spins} AWARDED`); await wait(1000); let total=0; for(let n=1;n<=spins;n+=1){ const r=this.reels.generate(beardBankConfig); const e=this.evaluator.evaluate(r.matrix,beardBankConfig,bet); this.say(`FREE SPIN ${n} OF ${spins} • 2×`); const host=this.root.querySelector<HTMLElement>("[data-reels]")!; await animateDomReels({host,finalColumns:r.matrix,rows:3,randomSymbol:()=>this.reels.generate(beardBankConfig).matrix[0]![0]!,profile:"bonus",renderSymbol:(id,reel)=>`<div class="bb60-symbol s-${id}" style="--reel:${reel}"><img src="${asset(id)}" alt="${LABELS[id]}"><small>${LABELS[id]}</small></div>`}); this.render(r.matrix); total += e.awardUnits*2; await wait(500); } await this.countAward(total,"VERNON FEATURE COMPLETE"); }
  private async livingVault(bet: number): Promise<void> {
    window.dispatchEvent(new CustomEvent("casino:sound",{detail:{cue:"vault"}}));
